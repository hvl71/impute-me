...
summarize_imputation<-function(
  ...
  crontabs<-sub(" .+$","",sub("^.+Rscript /srv/shiny-server/imputeme/","",crontabs))
...
crawl_for_snps_to_analyze<-function(uniqueIDs=NULL){
  ...
  for(module in list.files("/srv/shiny-server",full.names=T)){
  ...
  e<-try(load("/srv/shiny-server/nonsenser/2015-12-16_all_coding_SNPs.rdata"))
  ...
  load("/srv/shiny-server/AllDiseases/2019-03-04_all_gwas_snps.rdata")
  e1<-gwas_snps
  load("/srv/shiny-server/ukbiobank/2017-09-28_all_ukbiobank_snps.rdata")
  e2<-gwas_snps
  ...
  e<-try(load("/srv/shiny-server/ethnicity/2017-04-03_ethnicity_snps.rdata"))
  if(class(e)!="try-error"){
    for(uniqueID in uniqueIDs){
...
generate_report<-function(uniqueIDs=NULL, filename=NULL, updateProgress = updateProgress){
  ...
  filepath <- paste0("/srv/shiny-server/www/",filename)
...
run_export_script<-function(uniqueIDs=NULL,modules=NULL, delay=0){
  ...
  if(is.null(modules)){
    modules<-list.files("/srv/shiny-server/")
  }else{
    if(class(modules)!="character")stop("modules must be of class character")
    if(!all(file.exists(paste("/srv/shiny-server/",modules,sep=""))))stop("Not all modules given were found")
  }
  ...
  for(uniqueID in uniqueIDs){
    ...
    if(!"ethnicity"%in%colnames(pData)){
      source(paste(paste0("/srv/shiny-server/","ethnicity"  ,"/export_script.R")))
    ...
    for(module in modules){
      if(!file.info(paste0("/srv/shiny-server/",module))["isdir"])next
      if("export_script.R" %in% list.files(paste0("/srv/shiny-server/",module))){
        
        print(paste(Sys.time(),"Running module",module,"for",uniqueID))
        if(exists("export_function"))suppressWarnings(rm("export_function"))
        source(paste(paste0("/srv/shiny-server/",module,"/export_script.R")))
...
summarize_imputemany_json<-function(uniqueIDs, name){
  ...
  trait_path1 <- "/srv/shiny-server/AllDiseases/2019-03-04_trait_overview.xlsx"
  library(openxlsx)
  ...
  trait_path2 <- "/srv/shiny-server/ukbiobank/2017-09-28_trait_overoverview.rdata"
  load(trait_path2)
  ...
  file_out_long <- paste0("/srv/shiny-server/",file_out)
