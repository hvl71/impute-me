...
for(uniqueID in uniqueIDs){
  ...
  f1<-paste("/srv/shiny-server/www/",uniqueID,".simple_format.zip",sep="")
  f2<-paste("/srv/shiny-server/www/",uniqueID,".gen.zip",sep="")
  ...
  f6<-paste("/srv/shiny-server/www/",uniqueID,"_data.json",sep="")
...
unlink(list.files("/srv/shiny-server/www/",pattern="_report\\.pdf$",full.names=T))
...
age_limit <- 7
l<-data.frame(
  f=list.files("/srv/shiny-server/www",pattern="^summary_",full.names=T),
  stringsAsFactors = F
)
